...
intrinsic WriteBoundaryComponentsToGapFormat(File::MonStgElt,B::List,Elts::List)
 ...
 B:=[*[*[*[B[i][j][k][1],Position(Elts,matbas2([B[i][j][k][2]])[1])]:k in [1..#B[i][j]]*]:j in [1..#B[i]]*]:i in [1..#B]*]; 
 ...
 for i in [1..#B] do
  ...
  for j in [1..#B[i]] do
   ...
   for k in [1..#B[i][j]] do
    ...
    str cat:= "[";
    str cat:= IntegerToString(B[i][j][k][1]);
    str cat:= ",";
    str cat:= IntegerToString(B[i][j][k][2]);
    str cat:= "]";
   end for;
   str cat:= "]";
  end for;
...
intrinsic DimensionsToGapString(Dims::SeqEnum)-> MonStgElt
 ...
 str cat:= "DIMS:=[";
  for i in [1..#Dims] do
   ...
   str cat:= IntegerToString(Dims[i]);
  end for;
 str cat:= "];";
 return str;
end intrinsic;
